...
<c:set var="person2" value='<%=new Person("asd", 26) %>'></c:set>
<p>변경전</p>
<ul>
	<li>${person }</li>
	<li>${person2.name }</li>
...
<%
	...
	list.add(new Person("lgh", 28));
	list.add(new Person("lgh1", 26));
%>
...
<%
	...
	// ajax, fetch를 이용할 경우 ->
	// 데이터만 전달 -> 스크립트에서 데이터를 파싱하여 화면에 다시 그려야 함
	
	Map<String, Person> map = new HashMap<>();
	map.put("person1", new Person("사람1", 27));
	map.put("person2", new Person("사람2", 27));
%>
<!-- map은 변수로 선언만 하고 영역에 저장하지 않았기 때문에 EL 문법으로 사용할 수 없음 -->
